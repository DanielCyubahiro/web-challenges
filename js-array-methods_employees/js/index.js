...
function findByLastName(employees, lastName) {
  ...
  employees = employees.map(employee => {return {...employee, lastName: employee.lastName.toLowerCase()};});
...
function findByProfessionSubStringAndAge(employees, string, age) {
  return employees.find((employee) => {return employee.profession.includes(string) && employee.age >= age});
}
