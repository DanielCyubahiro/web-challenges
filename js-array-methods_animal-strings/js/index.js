...
const firstAnimalStartingWithLetterP = animalStrings.find((animal) => {return animal.startsWith('p')});
...
const animalsSortedAlphabetically = animalStrings.toSorted((a, b) => a.localeCompare(b));

// Hint: Guess what? There is a string method called `endsWith()`.
const anyAnimalEndsWithLetterZ = !!animalStrings.find((animal) => {return animal.endsWith('z')}) ;

const everyAnimalHasMoreThanTwoLetters = !!animalStrings.filter((animal) => {return animal.length > 2;});
