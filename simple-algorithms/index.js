...
function findLongestWord(words) {
  // TODO:
  //Create a variable that stores the longest word, that's starts out empty
  let longestWord = '';
  //Go through the array and compare each element with this variable
...
function sumNumbers(numbers) {
  ...
  //Using a loop go through the array adding one number to the sum at a time.
  let sum = 0;
...
function uniquifyArray(words) {
  ...
  //Go throught the main array, and check if the word is already in the new array
  words.forEach((word) => {
...
const mixedArray = [
  'apple', 'banana', 'cherry', 'date', 42, 7, 1995, 23, true, false];
...
function sum(array) {
  ...
  array.forEach((word) => {
    sum += typeof word === 'string' ? word.length : (typeof word ===
        'boolean') && word === true ? 1 : (typeof word === 'boolean') &&
    word === false ? 0 : word;
  });
  return sum;
}
